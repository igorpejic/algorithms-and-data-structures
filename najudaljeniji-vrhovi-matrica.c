...
void najudaljeniji_vrhovi(graph *g, int u) {
    ...
    int arr[MAXV];
    for (v = 0; v<g->nvertices; v++){
        if(g->distances[v] == max){
            arr[len] = v;
            len++;
        }
        else if(g->distances[v] > max){
            len = 1;
            arr[0] = v;
            max = g->distances[v];
        }
...
int main() {
    ...
    setbuf(stdout, NULL);
    do {
        DEBUG("\n1.Najudaljeniji vrhovi (najudaljniji_vrhovi)");
        DEBUG("\n2.Unos grafa\n3.Ispis grafa\n4.Izlaz\n");
        scanf("%d", &menu_choice);
        switch(menu_choice) {
            case 1:
